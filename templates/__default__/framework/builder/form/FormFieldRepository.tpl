		...
		add_field : function (field_id, id, field_nb) {
			if (this.integer <= this.max_input)
            {
				...
				jQuery('<div/>', {class: 'grouped-inputs flex-wide'}).appendTo('#' + new_field);
                    ...
                    jQuery('<a/>', {class : 'delete-item grouped-element bgc-full error', href : 'javascript:FormFieldRepository.delete_field('+ this.integer +');', id : 'delete_' + new_field, 'aria-label' : ${escapejs(@common.delete)}}).html('<i class="far fa-trash-alt" aria-hidden="true"></i>').appendTo('#' + new_field + ' .grouped-inputs');
    ...
    # START fieldelements #
        <li id="${escape(ID)}_{fieldelements.ID}">
            <div class="grouped-inputs flex-wide">
                ...
                <input class="input-label grouped-element" type="text" name="field_label_${escape(ID)}_{fieldelements.ID}" id="field_label_${escape(ID)}_{fieldelements.ID}" value="{fieldelements.LABEL}" placeholder="{@addon.modules.label}"/>
                <input class="input-url grouped-element" type="text" name="field_url_${escape(ID)}_{fieldelements.ID}" id="field_url_${escape(ID)}_{fieldelements.ID}" value="{fieldelements.URL}" placeholder="{@addon.modules.url}"/>
                <input class="input-directory grouped-element" type="text" name="field_directory_${escape(ID)}_{fieldelements.ID}" id="field_directory_${escape(ID)}_{fieldelements.ID}" value="{fieldelements.DIR}" placeholder="{@addon.modules.dir}"/>
                # IF fieldelements.C_DELETE #<a class="delete-item grouped-element bgc-full error# IF NOT C_DELETE # icon-disabled# ENDIF #" href="javascript:FormFieldRepository.delete_field({fieldelements.ID});" id="delete_${escape(ID)}_{fieldelements.ID}" aria-label="{@common.delete}"# IF C_DELETE # data-confirmation="delete-element"# ENDIF #><i class="far fa-trash-alt" aria-hidden="true"></i></a># ENDIF #
