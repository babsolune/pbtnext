 ...
 *      hc-nav- => push-nav- (trigger css class)
...
(function(global, factory) {
  if (typeof module === 'object' && typeof module.exports === 'object') {
    ...
    define('pushmenu', [], factory(global));
  }
  else {
    factory(global);
  }
  ...
  const pushmenu = function HcOffcanvasNav(elem, options) {
    options = options || {};
    ...
    const Helpers = pushmenu.Helpers;
    ...
    const Plugin = function($originalNav) {
      ...
      const navUniqId = `push-nav-${navCount}`;
      const Styles = Helpers.printStyle(`pushmenu-id-${navCount}-style`);
      const keydownEventName = 'keydown.pushmenu';
      ...
      if ($toggle && $toggle.length) {
        $toggle.forEach(($t) => {
          ...
          $t.classList.add('push-nav-trigger', navUniqId);
      ...
      const calcNav = () => {
        ...
        Styles.add(
          `.pushmenu.${navUniqId}.nav-position-left .nav-container`,
          `transform: translate3d(-${computedWidth}, 0, 0)`
        );
        Styles.add(
          `.pushmenu.${navUniqId}.nav-position-right .nav-container`,
          `transform: translate3d(${computedWidth}, 0, 0)`
        );
        Styles.add(
          `.pushmenu.${navUniqId}.nav-position-top .nav-container`,
          `transform: translate3d(0, -${computedHeight}, 0)`
        );
        Styles.add(
          `.pushmenu.${navUniqId}.nav-position-bottom .nav-container`,
          `transform: translate3d(0, ${computedHeight}, 0)`
        );
      ...
      const initNav = (reinit) => {
        ...
        // create styles
        Styles.add(`.pushmenu.${navUniqId}`, 'display: block', mediaquery);

        // hide original
        Styles.add(`.push-nav-original.${navUniqId}`, 'display: none', mediaquery);

        // trigger
        if (toggleDisplay) {
          Styles.add(`.push-nav-trigger.${navUniqId}`, `display: ${toggleDisplay && toggleDisplay !== 'none' ? toggleDisplay : 'inline-flex'}`, mediaquery);
        }

        if (['left', 'right'].indexOf(Settings.position) !== -1) {
          // container width
          Styles.add(`.pushmenu.${navUniqId} .nav-container`, `width: ${width}`);
        }
        else {
          // container height
          Styles.add(`.pushmenu.${navUniqId} .nav-container`, `height: ${height}`);
        }
        ...
        // wrappers
        Styles.add(
          `.pushmenu.${navUniqId}.nav-levels-overlap.nav-position-left li.level-open > .nav-wrapper`,
          `transform: translate3d(-${spacing}, 0, 0)`, mediaquery
        );
        Styles.add(
          `.pushmenu.${navUniqId}.nav-levels-overlap.nav-position-right li.level-open > .nav-wrapper`,
          `transform: translate3d(${spacing}, 0, 0)`, mediaquery
        );
        Styles.add(
          `.pushmenu.${navUniqId}.nav-levels-overlap.nav-position-top li.level-open > .nav-wrapper`,
          `transform: translate3d(0, -${spacing}, 0)`, mediaquery
        );
        Styles.add(
          `.pushmenu.${navUniqId}.nav-levels-overlap.nav-position-bottom li.level-open > .nav-wrapper`,
          `transform: translate3d(0, ${spacing}, 0)`, mediaquery
        );
      ...
      if (Settings.removeOriginalNav === true) {
        ...
        $originalNav.classList.add('push-nav-original', navUniqId);
      }
      ...
      function _openNav() {
        ...
        if (Settings.disableBody) {
          ...
          if (document.documentElement.scrollHeight > document.documentElement.clientHeight) {
            // has scroll bar
            html.classList.add('push-nav-yscroll');
          }

          document.body.classList.add('push-nav-open');
      ...
      function closeNav() {
        ...
        if (Settings.disableBody) {
          document.body.classList.remove('push-nav-open');
          html.classList.remove('push-nav-yscroll');
  ...
  if (typeof window.jQuery !== 'undefined') {
    const $ = window.jQuery;
    const namespace = 'pushmenu';

    $.fn.extend({
      pushmenu: function(args) {
        ...
        return this.each(function() {
          ...
          if (instance) {
            ...
            // create new instance
            instance = new pushmenu(this, args);
  ...
  // browser global
  window.pushmenu = window.pushmenu || pushmenu;
