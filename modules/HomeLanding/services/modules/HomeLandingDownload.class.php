 ...
 * @version     PHPBoost 6.1 - last update: 2023 02 20
...
class HomeLandingDownload
{
    public static function get_download_cat_view()
	{
        ...
        elseif (file_exists(PATH_TO_ROOT . '/modules/HomeLanding/templates/pagecontent/' . $module_cat . '.tpl'))
			$view = new FileTemplate('modules/HomeLanding/templates/pagecontent/' . $module_cat . '.tpl');
		else
    ...
    public static function get_download_view()
	{
		...
		$result = PersistenceContext::get_querier()->select('SELECT download.*, member.*, notes.average_notes, notes.notes_number, note.note, cat.rewrited_name AS rewrited_name_cat
		FROM ' . PREFIX . 'download download
