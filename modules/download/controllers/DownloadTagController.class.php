...
class DownloadTagController extends DefaultModuleController
{
	...
	private function build_sorting_form($field, $mode)
	{
		...
		$fieldset->add_field(new FormFieldSimpleSelectChoice('sort_fields', '', $field, $sort_options,
			['select_to_list' => true, 'events' => ['change' => 'document.location = "'. DownloadUrlBuilder::display_tag($this->get_keyword()->get_rewrited_name())->rel() . '" + HTMLForms.getField("sort_fields").getValue() + "/" + HTMLForms.getField("sort_mode").getValue();']]
		));
	...
	private function generate_response(HTTPRequestCustom $request)
	{
		...
		$graphical_environment->get_seo_meta_data()->set_description(StringVars::replace_vars($this->lang['download.seo.description.tag'], ['subject' => $this->get_keyword()->get_name()]), $page);
