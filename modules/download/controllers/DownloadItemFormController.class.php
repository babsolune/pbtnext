...
class DownloadItemFormController extends DefaultModuleController
{
    ...
    private function build_form(HTTPRequestCustom $request)
    {
        ...
        $form->set_layout_title($this->get_item()->get_id() === null ? $this->lang['download.add.item'] : ($this->is_duplication ? $this->lang['download.duplicate.item'] : $this->lang['download.edit.item']));
        ...
        $fieldset->add_field(new FormFieldDecimalNumberEditor('file_size', $this->lang['download.form.file.size'], $file_size,
            [
                ...
                'hidden' => ($request->is_post_method() ? $request->get_postbool(self::class . '_determine_file_size_automatically_enabled', false) : $this->is_file_size_automatic()),
            ]
        ));
        ...
        if (DownloadAuthorizationsService::check_authorizations($this->get_item()->get_id_category())->moderation()) {
            ...
            if (!$this->get_item()->is_published()) {
                $publication_fieldset->add_field(new FormFieldCheckbox('update_creation_date', $this->lang['form.update.creation.date'], false,
                    ['hidden' => $this->get_item()->get_publishing_state() != DownloadItem::NOT_PUBLISHED]
                ));
            }
            ...
            $publication_fieldset->add_field($publishing_start_date = new FormFieldDateTime('publishing_start_date', $this->lang['form.start.date'], ($this->get_item()->get_publishing_start_date() === null ? new Date() : $this->get_item()->get_publishing_start_date()),
                ['hidden' => ($request->is_post_method() ? ($request->get_postint(self::class . '_publication_state', 0) != DownloadItem::DEFERRED_PUBLICATION) : ($this->get_item()->get_publishing_state() != DownloadItem::DEFERRED_PUBLICATION))]
            ));
    ...
    private function is_contributor_member()
    {
        ...
        return (!DownloadAuthorizationsService::check_authorizations($id_category)->write() && DownloadAuthorizationsService::check_authorizations($id_category)->contribution());
    }
    ...
    private function save()
    {
        ...
        if ($this->form->get_value('determine_file_size_automatically_enabled')) {
            ...
            $units     = [$this->lang['common.unit.bytes'], $this->lang['common.unit.kilobytes'], $this->lang['common.unit.megabytes'], $this->lang['common.unit.gigabytes']];
    ...
    private function redirect()
    {
        ...
        if ($this->is_contributor_member()) {
            ...
            if ($this->is_new_item || $this->is_duplication) {
                AppContext::get_response()->redirect(DownloadUrlBuilder::display($category->get_id(), $category->get_rewrited_name(), $item->get_id(), $item->get_rewrited_title()), StringVars::replace_vars($this->lang['download.message.success.add'], ['title' => $item->get_title()]));
            } else {
                AppContext::get_response()->redirect(($this->form->get_value('referrer') ? $this->form->get_value('referrer') : DownloadUrlBuilder::display($category->get_id(), $category->get_rewrited_name(), $item->get_id(), $item->get_rewrited_title())), StringVars::replace_vars($this->lang['download.message.success.edit'], ['title' => $item->get_title()]));
            }

        } else {
            if ($this->is_new_item || $this->is_duplication) {
                AppContext::get_response()->redirect(DownloadUrlBuilder::display_pending(), StringVars::replace_vars($this->lang['download.message.success.add'], ['title' => $item->get_title()]));
            } else {
                AppContext::get_response()->redirect(($this->form->get_value('referrer') ? $this->form->get_value('referrer') : DownloadUrlBuilder::display_pending()), StringVars::replace_vars($this->lang['download.message.success.edit'], ['title' => $item->get_title()]));
            }
