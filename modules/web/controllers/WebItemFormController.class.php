...
class WebItemFormController extends DefaultModuleController
{
	...
	private function build_form(HTTPRequestCustom $request)
	{
		...
		$options_fieldset->add_field(new FormFieldCheckbox('partner', $this->lang['web.form.partner'], $this->get_item()->is_partner(), [
			'events' => ['click' => '
				if (HTMLForms.getField("partner").getValue()) {
					...
					if (HTMLForms.getField("partner_thumbnail").getValue())
						jQuery("#' . self::class . '_partner_thumbnail_preview").show();
				} else {
					HTMLForms.getField("partner_thumbnail").disable();
					jQuery("#' . self::class . '_partner_thumbnail_preview").hide();
		...
		$options_fieldset->add_field(new FormFieldUploadPictureFile('partner_thumbnail', $this->lang['web.form.partner.thumbnail'], $this->get_item()->get_partner_thumbnail()->relative(),
			['hidden' => ($request->is_post_method() ? !$request->get_postbool(self::class . '_partner', false) : !$this->get_item()->is_partner())]
		));
		...
		if (CategoriesAuthorizationsService::check_authorizations($this->get_item()->get_id_category())->moderation())
		{
			...
			if (!$this->get_item()->is_published())
			{
				$publication_fieldset->add_field(new FormFieldCheckbox('update_creation_date', $this->lang['form.update.creation.date'], false,
					['hidden' => $this->get_item()->get_status() != WebItem::NOT_PUBLISHED]
				));
			}
			...
			$publication_fieldset->add_field($publishing_start_date = new FormFieldDateTime('publishing_start_date', $this->lang['form.start.date'], ($this->get_item()->get_publishing_start_date() === null ? new Date() : $this->get_item()->get_publishing_start_date()),
				['hidden' => ($request->is_post_method() ? ($request->get_postint(self::class . '_publication_state', 0) != WebItem::DEFERRED_PUBLICATION) : ($this->get_item()->get_publishing_state() != WebItem::DEFERRED_PUBLICATION))]
			));
	...
	private function contribution_actions(WebItem $item)
	{
		if ($this->is_contributor_member())
		{
			...
			HooksService::execute_hook_action($this->is_new_item || $this->is_duplication ? 'add_contribution' : 'edit_contribution', self::$module_id, array_merge($item->get_properties(), ['item_url' => $item->get_item_url()]));
		}
		else
		{
	...
	private function redirect()
	{
		...
		{
			if ($this->is_new_item || $this->is_duplication)
				AppContext::get_response()->redirect(WebUrlBuilder::display($category->get_id(), $category->get_rewrited_name(), $item->get_id(), $item->get_rewrited_title()), StringVars::replace_vars($this->lang['web.message.success.add'], ['title' => $item->get_title()]));
			else
				AppContext::get_response()->redirect(($this->form->get_value('referrer') ? $this->form->get_value('referrer') : WebUrlBuilder::display($category->get_id(), $category->get_rewrited_name(), $item->get_id(), $item->get_rewrited_title())), StringVars::replace_vars($this->lang['web.message.success.edit'], ['title' => $item->get_title()]));
		}
		else
		{
			if ($this->is_new_item || $this->is_duplication)
				AppContext::get_response()->redirect(WebUrlBuilder::display_pending(), StringVars::replace_vars($this->lang['web.message.success.add'], ['title' => $item->get_title()]));
			else
				AppContext::get_response()->redirect(($this->form->get_value('referrer') ? $this->form->get_value('referrer') : WebUrlBuilder::display_pending()), StringVars::replace_vars($this->lang['web.message.success.edit'], ['title' => $item->get_title()]));
		}
	}
