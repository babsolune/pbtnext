...
class PbtManagerGitHubService
{
    ...
    {
        $url = 'https://github.com/' . urlencode($owner) . '/' . urlencode($repo) . '/archive/refs/heads/' . urlencode($branch) . '.zip';
    ...
    private static function github_get($url)
    {
        ...
        // GitHub renvoie {"message": "..."} en cas d'erreur API (rate limit, repo introuvable…)
    ...
    private static function http_get($url, $binary = false)
    {
        ...
        $headers = array(
            'User-Agent: PHPBoost-PbtManager/1.0',
        ...
        if (function_exists('curl_init'))
        {
            ...
            if ($result === false)
                return false; // erreur réseau cURL

            if ($http_code >= 400)
                return false; // erreur HTTP (401, 403, 404…)
