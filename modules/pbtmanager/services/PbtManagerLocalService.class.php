...
class PbtManagerLocalService
{
    ...
    {
        $modules_dir = PATH_TO_ROOT . '/modules/';
        $result      = array();
        ...
        foreach ($dirs as $dir)
        {
            ...
            // Try to get the display name from the module configuration
            if ($installed)
            {
                try {
            ...
            {
                // Module folder exists but not registered: try config.ini
    ...
    {
        $ini_path = PATH_TO_ROOT . '/modules/' . $module_id . '/config.ini';
        if (!file_exists($ini_path))
            return null;
    ...
    {
        // 1. Download zip
        $zip_content = PbtManagerGitHubService::download_branch_zip($owner, $repo, $branch);
        if ($zip_content === false)
            return 'Impossible de télécharger le dépôt depuis GitHub.';

        // 2. Save to a temp file
        $tmp_zip = tempnam(sys_get_temp_dir(), 'pbtm_') . '.zip';
        if (file_put_contents($tmp_zip, $zip_content) === false)
            return 'Impossible d\'écrire le fichier temporaire.';
        ...
        $target_dir = PATH_TO_ROOT . '/modules/' . $module_name . '/';
        ...
        if ($extracted === 0)
            return 'Aucun fichier extrait. Vérifiez le nom du module et le chemin du dépôt.';
