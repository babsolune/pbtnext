...
class AdminPbtManagerConfigController extends DefaultAdminModuleController
{
    ...
    private function build_form()
    {
        ...
        // --- GitHub token ---
        $fieldset_token = new FormFieldsetHTML('token_fieldset', $lang['pbtmanager.config.github.token']);
        $form->add_fieldset($fieldset_token);
        $fieldset_token->add_field(new FormFieldTextEditor(
            'github_token',
            $lang['pbtmanager.config.github.token'],
            $config->get_github_token(),
            array('description' => 'ghp_… — optionnel, permet d\'augmenter la limite de taux GitHub API (60 → 5 000 req/h)')
        ));

        // --- Repos ---
        $fieldset_repos = new FormFieldsetHTML('repos_fieldset', $lang['pbtmanager.config.repos']);
        ...
        $url_repos  = PbtManagerUrlBuilder::ajax_repos()->rel();

        $l_add      = addslashes($lang['pbtmanager.config.repo.add']);
        $l_delete   = addslashes($lang['pbtmanager.config.repo.delete']);
        $l_org      = addslashes($lang['pbtmanager.config.repo.org']);
        $l_pick     = addslashes($lang['pbtmanager.config.repo.pick']);
        $l_path     = addslashes($lang['pbtmanager.config.repo.path']);
        $l_label    = addslashes($lang['pbtmanager.config.repo.label']);
        $l_loading  = addslashes($lang['pbtmanager.remote.loading']);
        $l_error    = addslashes($lang['pbtmanager.remote.error']);
...
(function(){
    ...
    function renderList() {
        ...
        list.innerHTML = '';
        if (!repos.length) {
            list.innerHTML = '<p style="color:#6b7280;font-style:italic;margin:.5rem 0;">Aucun dépôt configuré.</p>';
        }
    ...
    document.getElementById('pbtm-add-repo-btn').addEventListener('click', function() {
        ...
        if (!org || !repo) { fb.textContent = 'Sélectionnez un dépôt.'; fb.style.display='block'; return; }
...
HTML;
    ...
    private function save()
    {
        ...
        $config->set_github_token(trim($this->form->get_value('github_token')));
