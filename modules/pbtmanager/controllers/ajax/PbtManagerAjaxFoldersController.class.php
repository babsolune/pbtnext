 ...
 */
class PbtManagerAjaxFoldersController extends AbstractController
{
    public function execute(HTTPRequestCustom $request)
    {
        if (!PbtManagerAuthorizationsService::check_authorizations()->read())
        ...
        $folders = PbtManagerGitHubService::get_modules_with_versions($owner, $repo, $branch, $path);

        if (!is_array($folders))
            return new JSONResponse(array('success' => false, 'error' => 'GitHub API error'));

        $local  = PbtManagerLocalService::get_local_modules();
        $result = array();
