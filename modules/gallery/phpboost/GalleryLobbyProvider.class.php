...
class GalleryLobbyProvider extends DefaultLobbyModuleProvider
{
	...
	public function get_view(): FileTemplate
	{
		...
		$result = PersistenceContext::get_querier()->select(
			'SELECT g.id, g.id_category, g.name, g.path, g.timestamp, g.aprob, g.width, g.height, g.user_id, g.views,
		...
		while ($row = $result->fetch())
		{
			$view->assign_block_vars('items', [
				...
				'U_ITEM'       => PATH_TO_ROOT . '/modules/gallery/' . url('picture.php?id=' . $row['id'], $row['id'] . '.php'),
			]);
		}
		$result->dispose();
