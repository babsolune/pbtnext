 ...
 * @return string|bool Returns 'wrong_site' or 'wrong_version' if invalid, true otherwise.
 */
function check_backup_file(File $file): string|bool
{
    ...
    if (preg_match("`'kernel-general-config',`u", $file_content))
    {
        ...
        elseif (!preg_match('`s:16:"phpboost_version";s:' . strlen($general_config->get_phpboost_major_version()) . ':"' . $general_config->get_phpboost_major_version() . '";`u', $file_content))
        {
...
if ($action == 'backup_table' && !empty($get_table))
{
    $tables_backup = true;
}
...
if (!empty($query))
{
    ...
    if (!empty($query))
    {
        ...
        foreach (explode(';', $query) as $q)
        {
            ...
            if (TextHelper::strtolower(TextHelper::substr($q, 0, 6)) == 'select')
            {
                try
                {
            ...
            elseif (TextHelper::substr($lower_query, 0, 11) == 'insert into' || TextHelper::substr($lower_query, 0, 6) == 'update' || TextHelper::substr($lower_query, 0, 11) == 'delete from' || TextHelper::substr($lower_query, 0, 11) == 'alter table'  || TextHelper::substr($lower_query, 0, 8) == 'truncate' || TextHelper::substr($lower_query, 0, 10) == 'drop table')
            {
                try
                {
...
elseif ($action == 'restore')
{
        ...
        if (preg_match('`[^/]+\.zip$`u', $filename))
        {
            ...
            if ($zip->open($file_path) === TRUE)
            {
                for ($i = 0; $i < $zip->numFiles; $i++)
                {
                    ...
                    if (preg_match('`[^/]+\.sql$`u', $stat['name']))
                    {
                        $extract_filename = $stat['name'];
                        break;
                    }
                }

                if ($extract_filename && $zip->extractTo(PATH_TO_ROOT . '/cache/backup/', $extract_filename))
                {
    ...
    foreach ($backup_folder->get_files() as $file)
    {
        ...
        {
            $filelist[$file->get_last_modification_date()] = [
                'file_name' => $file->get_name(),
                'weight' => File::get_formated_size($file->get_file_size()),
                'file_date' => Date::to_format($file->get_last_modification_date(), Date::FORMAT_DAY_MONTH_YEAR)
            ];
        }
    }
...
else
{
    if ($action == 'backup')
    {
        ...
        {
            $zip = new ZipArchive;
            if ($zip->open(PATH_TO_ROOT . '/cache/backup/' . $file_basename . '.zip', ZipArchive::CREATE) === TRUE)
            {
                $zip->addFile(PATH_TO_ROOT . '/cache/backup/' . $file_basename . '.sql', $file_basename . '.sql');
                $zip->close();
    ...
    if ($tables_backup)
    {
        ...
        foreach ($tables as $table)
        {
            ...
            $view->assign_block_vars('table_list', [
                'NAME' => $table,
    ...
    else
    {
        ...
        list($nbr_rows, $nbr_data, $nbr_free) = [0, 0, 0];
        ...
        $view->put_all([
            ...
            'L_RESTORE_UPLOADED_FILE_SIZE_EXCEEDED' => StringVars::replace_vars(LangLoader::get_message('upload.warning.file.size', 'upload-lang'), ['max_file_size' => File::get_formated_size($upload_max_filesize)]),
