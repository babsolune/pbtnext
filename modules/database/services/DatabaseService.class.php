...
class DatabaseService
{
	...
	public static function indent_query($query)
	{
		...
		$query = preg_replace('`\b(' . implode('|', ['select', 'update', 'insert into', 'from', 'left join', 'right join', 'cross join', 'natural join', 'inner join', 'left outer join', 'right outer join', 'full outer join', 'full join', 'drop', 'truncate', 'where', 'order by', 'group by', 'limit', 'having', 'union']) . ')+`u', "\r\n" . '$1', $query);

		//Case des mots clés.
		$key_words = ['select', 'update', 'delete', 'insert into', 'truncate', 'alter', 'table', 'status', 'set', 'drop', 'from', 'values', 'count', 'distinct', 'having', 'left', 'right', 'join', 'natural', 'outer', 'inner', 'between', 'where', 'group by', 'order by', 'limit', 'union', 'or', 'and', 'not', 'in', 'as', 'on', 'all', 'any', 'like', 'concat', 'substring', 'collate', 'collation', 'primary', 'key', 'default', 'null', 'exists', 'status', 'show'];
	...
	public static function highlight_query($query)
	{
		...
		$query = preg_replace('`(' . implode('|', array_map('preg_quote', ['*', '=', ',', '!=', '<>', '>', '<', '.', '(', ')'])) . ')+`uU', '<span class="db-operator-color">$1</span>', $query);
