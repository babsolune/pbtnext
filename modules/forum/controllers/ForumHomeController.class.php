...
class ForumHomeController extends DefaultModuleController
{
	...
	private function build_view()
	{
		...
		require_once(PATH_TO_ROOT . '/modules/forum/forum_begin.php');
		require_once(PATH_TO_ROOT . '/modules/forum/forum_tools.php');
			...
			else //On liste les sous-catégories
			{
				...
				if (($display_sub_cats || !empty($id_get)) && !in_array($row['cid'], $is_sub_forum))
				{
					...
					$this->view->assign_block_vars('forums_list.subcats', array_merge(
						...
						'U_LAST_TOPIC'         => Url::to_rel("/forum/topic" . url('.php?id=' . $row['tid'], '-' . $row['tid'] . '-' . Url::encode_rewrite($row['title']) . '.php')),
						'U_LAST_MESSAGE'       => !empty($row['last_topic_id']) ? Url::to_rel("/forum/topic" . url('.php?' . $last_page . 'id=' . $row['tid'], '-' . $row['tid'] . $last_page_rewrite . '-' . Url::encode_rewrite($row['title']) . '.php')) . '#m' . $last_msg_id : '',
