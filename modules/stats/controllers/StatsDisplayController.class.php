...
class StatsDisplayController extends DefaultModuleController
{
    ...
    private function check_erase(HTTPRequestCustom $request)
    {
        ...
        {
            $file = new File(PATH_TO_ROOT . '/modules/stats/cache/robots.txt');
            if ($file->exists())
                $file->delete();
        }
        ...
        if ($erase_occasional) //Erase occasional robots
        {
            ...
            $file = @fopen(PATH_TO_ROOT . '/modules/stats/cache/robots.txt', 'r+');
    ...
    private function affect_vars(HTTPRequestCustom $request)
    {
        ...
        // Days number for each month (with leap years)
    ...
    {
        // Display all visitors of the active day
        ...
        if ($this->year_requested && !$this->month_requested) // Visits per month sorted by years.
        {
            // Get days number in table and the best of the month
            ...
            while ($row = $result->fetch())
            {
                ...
                // display the stats numerically in a table below
    ...
    private function build_pages_view()
    {
        ...
        // look for the number of days present in the table, and the monthly record
        $info = ['max_nbr' => 0, 'min_day' => 0, 'sum_nbr' => 0, 'avg_nbr' => 0];
        try {
        ...
        // Display the total visitors and of the day
    ...
    private function build_robots_view()
    {
        ...
        {
            if (is_array($value)) {
                // Safely get visits_number with default 0
                $visits = isset($value['visits_number']) ? (int)$value['visits_number'] : 0;
            } else {
                // Old scalar format
                $visits = (int)$value;
            }

            $robots_visits[$key] = $visits;
            $robots_visits_number += $visits;
        }
        ...
        if ($robots_visits_number)
        {
            foreach ($robots_visits as $key => $value)
            {
                $this->view->assign_block_vars('list', [
                    ...
                    'COLOR'         => $key ? $dataset->get_color_label($key) : '#000000',
                    'VISITS_NUMBER' => $value,
                    'LAST_SEEN'     => (
                        is_array($array_robot[$key])
                        && isset($array_robot[$key]['last_seen'])
                    )
                        ? Date::to_format($array_robot[$key]['last_seen'], Date::FORMAT_DAY_MONTH_YEAR)
                        : $this->lang['common.undetermined'],
                    'PERCENT'       => $robots_visits_number > 0
                        ? NumberHelper::round($value / $robots_visits_number * 100, 1)
                        : 0,
                    'L_NAME'        => $key,
