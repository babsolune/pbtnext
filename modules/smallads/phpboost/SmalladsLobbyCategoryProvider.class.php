...
class SmalladsLobbyCategoryProvider extends DefaultLobbyCategoryProvider
{
	public function get_view(): FileTemplate
	{
		...
		$categories = $lobby_module->is_subcategories_content_displayed() ? CategoriesService::get_authorized_categories($lobby_module->get_id_category(), $module_config->are_summaries_displayed_to_guests(), $phpboost_id) : [$lobby_module->get_id_category()];
		...
		$view->put_all([
			...
			'L_MODULE_TITLE'     => ModulesManager::get_module($phpboost_id)->get_configuration()->get_name(),
