 ...
 * @version     PHPBoost 6.1 - last update: 2022 04 19
...
abstract class AbstractServerStatusServer
{
	...
	{
		return Url::to_rel('/modules/ServerStatus/templates/images/icons/32/' . $this->id . '.png');
	}

	public function has_medium_icon()
	{
		$file = new File(PATH_TO_ROOT . '/modules/ServerStatus/templates/images/icons/32/' . $this->id . '.png');
		return $file->exists();
	}

	public function get_large_icon()
	{
		return Url::to_rel('/modules/ServerStatus/templates/images/icons/64/' . $this->id . '.png');
	}

	public function has_large_icon()
	{
		$file = new File(PATH_TO_ROOT . '/modules/ServerStatus/templates/images/icons/64/' . $this->id . '.png');
		return $file->exists();
	}
	...
	public function check_status($force_check = false)
	{
		...
		if ((($this->get_last_check_time() + ($config->get_refresh_delay() * 60)) < time()) || $force_check)
		{
			...
			@stream_set_timeout($this->Socket, intval($timeout));
