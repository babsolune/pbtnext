 ...
 * @version     PHPBoost 6.1 - last update: 2018 04 24
...
class SocialNetworksConfig extends AbstractConfigData
{
	...
	{
		return [
			self::AUTHENTICATIONS_ENABLED    => [],
			self::CLIENT_IDS                 => [],
			self::CLIENT_SECRETS             => [],
			self::CONTENT_SHARING_ENABLED    => [FacebookSocialNetwork::SOCIAL_NETWORK_ID, TwitterSocialNetwork::SOCIAL_NETWORK_ID],
			self::SOCIAL_NETWORKS_ORDER      => [],
			self::ADDITIONAL_SOCIAL_NETWORKS => []
		];
	}
	...
	{
		return ConfigManager::load(self::class, 'social-networks', 'config');
	}
