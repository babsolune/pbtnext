...
class VideoTagController extends DefaultModuleController
{
	private $keyword;
	...
	public function build_view(HTTPRequestCustom $request)
	{
		...
		LEFT JOIN ' . DB_TABLE_KEYWORDS_RELATIONS . ' relation ON relation.module_id = \'video\' AND relation.id_in_module = video.id
	...
	private function build_sorting_form($field, $mode)
	{
		...
		$fieldset->add_field(new FormFieldSimpleSelectChoice('sort_fields', '', $field, $sort_options,
			array('select_to_list' => true, 'events' => array('change' => 'document.location = "'. VideoUrlBuilder::display_tag($this->get_keyword()->get_rewrited_name())->rel() . '" + HTMLForms.getField("sort_fields").getValue() + "/" + HTMLForms.getField("sort_mode").getValue();'))
		));
	...
	private function get_pagination($condition, $parameters, $field, $mode, $page)
	{
		...
		$pagination = new ModulePagination($page, $result['items_number'], (int)VideoConfig::load()->get_items_per_page());
		$pagination->set_url(VideoUrlBuilder::display_tag($this->get_keyword()->get_rewrited_name(), $field, $mode, '%d'));
	...
	private function generate_response(HTTPRequestCustom $request)
	{
		...
		$graphical_environment->set_page_title($this->get_keyword()->get_name(), $this->lang['video.module.title'], $page);
		$graphical_environment->get_seo_meta_data()->set_description(StringVars::replace_vars($this->lang['video.seo.description.tag'], array('subject' => $this->get_keyword()->get_name())), $page);
		$graphical_environment->get_seo_meta_data()->set_canonical_url(VideoUrlBuilder::display_tag($this->get_keyword()->get_rewrited_name(), $sort_field, $sort_mode, $page));

		$breadcrumb = $graphical_environment->get_breadcrumb();
		$breadcrumb->add($this->lang['video.module.title'], VideoUrlBuilder::home());
		$breadcrumb->add($this->get_keyword()->get_name(), VideoUrlBuilder::display_tag($this->get_keyword()->get_rewrited_name(), $sort_field, $sort_mode, $page));
