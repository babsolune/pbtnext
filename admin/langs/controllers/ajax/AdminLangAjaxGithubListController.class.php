...
class AdminLangAjaxGithubListController extends AbstractController
{
	public function execute(HTTPRequestCustom $request)
	{
		...
		foreach ($dirs as $item)
		{
			...
			if (empty($config) || (isset($config['addon_type']) ? $config['addon_type'] : '') !== 'lang')
				continue;

			$thumb_path = ($path !== '' ? $path . '/' : '') . $addon_id . '/flag.png';
			$addons[] = [
				'id'            => $addon_id,
				'name'          => isset($config['name'])           ? $config['name']          : $addon_id,
