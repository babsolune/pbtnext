 ...
 * @version     PHPBoost 6.1 - last update: 2022 05 19
...
class AdminMemberAddController extends DefaultAdminController
{
	public function execute(HTTPRequestCustom $request)
	{
		...
		if ($this->submit_button->has_been_submited() && $this->form->validate())
		{
			...
			AppContext::get_response()->redirect(($this->form->get_value('referrer') ? $this->form->get_value('referrer') : AdminMembersUrlBuilder::management()), StringVars::replace_vars($this->lang['user.message.success.add'], ['name' => $display_name]));
		}
	...
	private function build_form(HTTPRequestCustom $request)
	{
		...
		$fieldset->add_field($display_name = new FormFieldTextEditor('display_name', $this->lang['user.display.name'], '',
			...
			[new FormFieldConstraintLengthRange(3, 100), new FormFieldConstraintDisplayNameExists()]
		));

		$fieldset->add_field($email = new FormFieldMailEditor('email', $this->lang['user.email'], '',
			['required' => true],
			[new FormFieldConstraintMailExist()]
		));
		...
		$fieldset->add_field($password = new FormFieldPasswordEditor('password', $this->lang['user.password'], '',
			['required' => true, 'autocomplete' => false, 'hidden' => true],
	...
	private function save()
	{
		...
		if ($user_id)
		{
			...
			HooksService::execute_hook_action('add_user', 'user', array_merge($user->get_properties(), ['title' => $user->get_display_name(), 'url' => UserUrlBuilder::profile($user_id)->rel()]));
		}
