...
class AdminViewAllMembersController extends DefaultAdminController
{
	...
	private function build_table()
	{
		...
		$table_model->add_filter(new HTMLTableEqualsFromListSQLFilter('approved', 'filter8', $this->lang['user.approbation'], [1 => $this->lang['common.status.approved'], 0 => $this->lang['common.status.unapproved']]));
		...
		foreach ($result as $row)
		{
			...
			if ($user->get_level() != User::ADMINISTRATOR_LEVEL || ($user->get_level() == User::ADMINISTRATOR_LEVEL && $number_admins > 1))
				...
				$delete_link = new DeleteLinkHTMLElement('', '', ['disabled' => true]);
			...
			$results[] = new HTMLTableRow([
				...
				new HTMLTableRowCell(new LinkHTMLElement('mailto:' . $user->get_email(), '<i class="fa fa-fw iboost fa-iboost-email"></i>', ['aria-label' => $this->lang['user.email']], 'button submit')),
