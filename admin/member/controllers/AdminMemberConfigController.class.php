 ...
 * @version     PHPBoost 6.1 - last update: 2024 06 07
...
class AdminMemberConfigController extends DefaultAdminController
{
	...
	private function build_form()
	{
		...
		// 	[
		// 		new FormFieldSelectChoiceOption($this->lang['form.display.type.grid'], UserAccountsConfig::GRID_VIEW, ['data_option_icon' => 'fa fa-th-large']),
		// 		new FormFieldSelectChoiceOption($this->lang['form.display.type.table'], UserAccountsConfig::TABLE_VIEW, ['data_option_icon' => 'fa fa-table'])
		// 	],
		// 	[
		// 		'select_to_list' => true,
		// 		'events' => ['change' => '
		...
		$fieldset->add_field(new FormFieldMultiLineTextEditor('forbidden_mail_domains', $this->lang['user.forbidden.email.domains'], implode(',', $this->security_config->get_forbidden_mail_domains()),
			['description' => $this->lang['user.forbidden.email.domains.clue']]
		));
		...
		$fieldset->add_field(new FormFieldNumberEditor('maximal_width_avatar', $this->lang['user.avatar.max.width'], $this->user_accounts_config->get_max_avatar_width(),
			['description' => $this->lang['user.avatar.max.width.clue']],
			[new FormFieldConstraintRegex('`^[0-9]+$`iu')]
		));
		...
		$fieldset->add_field(new FormFieldNumberEditor('maximal_height_avatar', $this->lang['user.avatar.max.height'], $this->user_accounts_config->get_max_avatar_height(),
			['description' => $this->lang['user.avatar.max.height.clue']],
			[new FormFieldConstraintRegex('`^[0-9]+$`iu')]
		));
		...
		$auth_settings = new AuthorizationsSettings([new ActionAuthorization($this->lang['user.authorization.description'], UserAccountsConfig::AUTH_READ_MEMBERS_BIT)]);
	...
	private function save()
	{
		...
		$this->security_config->set_forbidden_mail_domains($this->form->get_value('forbidden_mail_domains') ? explode(',', str_replace([' ', ';'], ['', ','], $this->form->get_value('forbidden_mail_domains'))) : []);
		...
		HooksService::execute_hook_action('edit_config', 'kernel', ['title' => $this->lang['user.members.config'], 'url' => AdminMembersUrlBuilder::configuration()->rel()]);
	}
}
?>
