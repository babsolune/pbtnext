 ...
 * @version     PHPBoost 6.1 - last update: 2022 01 15
...
gd library			: " . (int)$server_configuration->has_gd_extension() . "
curl extension			: " . (int)$server_configuration->has_curl_extension() . "
mbstring extension		: " . (int)$server_configuration->has_mbstring_extension() . "
...
";
        ...
        $fieldset->add_field(new FormFieldFree('gd_extension', $this->lang['admin.gd.extension'], $server_configuration->has_gd_extension() ? $picture_yes : $picture_no));
        $fieldset->add_field(new FormFieldFree('curl_extension', $this->lang['admin.curl.extension'], $server_configuration->has_curl_extension() ? $picture_yes : $picture_no));
        $fieldset->add_field(new FormFieldFree('mbstring_extension', $this->lang['admin.mbstring.extension'], $server_configuration->has_mbstring_extension() ? $picture_yes : $picture_no));
        $fieldset->add_field(new FormFieldFree('zip_extension', $this->lang['admin.zip.extension'], $server_configuration->has_zip_extension() ? $picture_yes : $picture_no));
...
";
    ...
    public static function get_advice(HTMLForm $html_form)
    {
        ...
        if (ModulesManager::is_module_installed('qaptcha') && ModulesManager::is_module_activated('qaptcha') && ContentManagementConfig::load()->get_used_captcha_module() == 'qaptcha' && QaptchaConfig::load()->count_items() < 3)
            $fieldset->add_field(new FormFieldFree('qaptcha_questions_number', '', MessageHelper::display(LangLoader::get_message('advices.questioncaptcha.items.number', 'common', 'qaptcha'), MessageHelper::WARNING)->render()));
