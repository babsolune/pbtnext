...
{
    private CSSCacheConfig $css_cache_config;

    public function execute(HTTPRequestCustom $request): AdminCacheMenuDisplayResponse
    {
        $this->init();
    ...
    protected function build_form(): void
    {
        ...
        $fieldset->add_field(new FormFieldBooleanInformation('apc_available', $this->lang['admin.apc.available'], $this->is_apc_available(),
            [
                'class' => 'top-field',
                'description' => $this->lang['admin.apc.available.clue']
            ]
        ));
    ...
    private function is_apc_available(): bool
    {
        return DataStoreFactory::is_apc_available();
    }

    private function is_apc_enabled(): bool
    {
    ...
    protected function save(): void
    {
        ...
        HooksService::execute_hook_action('edit_config', 'kernel', [
            'title' => $this->lang['admin.cache.configuration'],
            'url' => AdminCacheUrlBuilder::configuration()->rel()
        ]);
    }
}
?>
