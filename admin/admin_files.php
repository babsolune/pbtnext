...
{
    try {
        $parent_folder = PersistenceContext::get_querier()->select_single_row(PREFIX . 'upload_cat', ['id_parent', 'user_id'], 'WHERE id=:id', ['id' => $parent_folder]);
...
elseif (!empty($_FILES['upload_file']['name'])) // Add file
{
    ...
    if (is_writable($dir)) // Folder is writable => upload is available
    {
        ...
        else // Insert in database
        {
            ...
            try {
                $user_id = PersistenceContext::get_querier()->get_column_value(DB_TABLE_UPLOAD_CAT, 'user_id', 'WHERE id=:id', ['id' => $folder]);
            } catch (RowNotFoundException $e) {
            ...
            {
                $result = PersistenceContext::get_querier()->insert(DB_TABLE_UPLOAD, ['shared' => $is_shared_checkbox, 'idcat' => $folder, 'name' => $parameters['name'], 'path' => $parameters['path'], 'user_id' => $user_id, 'size' => $parameters['size'], 'type' => $parameters['extension'], 'timestamp' => time()]);
...
elseif (!empty($move_folder) && $to != -1) // Moving folder
{
    ...
    $user_id = AppContext::get_current_user()->get_id();
    try {
        $user_id = PersistenceContext::get_querier()->get_column_value(DB_TABLE_UPLOAD_CAT, 'user_id', 'WHERE id=:id', ['id' => $move_folder]);
    } catch (RowNotFoundException $e) {
...
elseif (!empty($move_folder) || !empty($move_file))
{
    ...
    {
        try {
            $folder_info = PersistenceContext::get_querier()->select_single_row(PREFIX . 'upload_cat', ['name', 'id_parent'], 'WHERE id=:id', ['id' => $move_folder]);
        } catch (RowNotFoundException $e) {
    ...
    {
        try {
            $info_move = PersistenceContext::get_querier()->select_single_row(PREFIX . 'upload', ['path', 'name', 'type', 'size', 'idcat'], 'WHERE id=:id', ['id' => $move_file]);
        } catch (RowNotFoundException $e) {
            $error_controller = PHPBoostErrors::unexisting_element();
            DispatchManager::redirect($error_controller);
        }
...
else
{
    ...
    // Errors management
    $array_error = ['e_upload_no_file', 'e_upload_invalid_format', 'e_upload_max_weight', 'e_upload_error', 'e_upload_failed_unwritable', 'e_unlink_disabled'];
    ...
    {
        $result = PersistenceContext::get_querier()->select("
                SELECT id, name, id_parent, user_id
    ...
    foreach ($types as $loop_id => $where_clause)
    {
        ...
        if (!empty($folder_member) && empty($folder) || $show_shared)
        {
                $result = PersistenceContext::get_querier()->select("
                        SELECT
                            up.id, up.shared, up.name, up.path, up.size, up.type, up.timestamp,
                            m.user_id, m.display_name, m.level, m.user_groups
        ...
        else
        {
            $result = PersistenceContext::get_querier()->select("
                    ...
                    FROM " . DB_TABLE_UPLOAD . " up
                    LEFT JOIN " . DB_TABLE_MEMBER . " m ON m.user_id = up.user_id
                    WHERE idcat = :id" . ((empty($folder) || $folder_info['user_id'] <= 0) ? ' AND up.user_id = -1' : ' AND up.user_id != -1')
                , [
                    'id' => $folder
                ]
            );
