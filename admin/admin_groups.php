...
if ($valid && !empty($idgroup_post)) // group modification
{
    ...
    $group_auth = ['auth_flood' => $auth_flood, 'pm_group_limit' => $pm_group_limit, 'data_group_limit' => $data_group_limit];
    PersistenceContext::get_querier()->update(
        ...
        [
            ...
            'img' => $img,
            'color' => $color_group,
            'auth' => TextHelper::serialize($group_auth)
        ],
        'WHERE id = :id',
        ['id' => $idgroup_post]
    );
...
elseif ($valid && $add_post) // Add group
{
    ...
    if (!empty($name))
    {
        if (!GroupsCache::load()->group_name_exists($name))
        {
            // Insert in database
            $group_auth = ['auth_flood' => $auth_flood, 'pm_group_limit' => $pm_group_limit, 'data_group_limit' => $data_group_limit];
            $result = PersistenceContext::get_querier()->insert(
                DB_TABLE_GROUP,
                [
                    'name' => $name,
                    'img' => $img,
...
elseif (!empty($idgroup) && $del_group) // Delete group.
{
    $array_members = [];
    try {
        $array_members = explode('|', PersistenceContext::get_querier()->get_column_value(DB_TABLE_GROUP, 'members', 'WHERE id=:id', ['id' => $idgroup]));
    ...
    if(!empty($array_members))
    {
        ...
        {
            if (!empty($user_id) && UserService::user_exists('WHERE user_id=:user_id', ['user_id' => $user_id]))
                GroupsService::remove_member($user_id, $idgroup); // Updating the list of members being in the deleted group.
        }
    }

    PersistenceContext::get_querier()->delete(DB_TABLE_GROUP, 'WHERE id=:id', ['id' => $idgroup]);
...
elseif (!empty($idgroup) && $add_mbr) // Add member to group
{
    ...
    $user_id = 0;
    try {
        $user_id = PersistenceContext::get_querier()->get_column_value(DB_TABLE_MEMBER, 'user_id', 'WHERE display_name=:login', ['login' => $login]);
    } catch (RowNotFoundException $e) {}
...
elseif (!empty($idgroup)) // Group editing interface
{
    ...
    try {
        $group = PersistenceContext::get_querier()->select_single_row(DB_TABLE_GROUP, ['id', 'name', 'img', 'color', 'auth', 'members'], 'WHERE id=:id', ['id' => $idgroup]);
    } catch (RowNotFoundException $e) {
    ...
    if (!empty($group['id']))
    {
        ...
        $members = '';
        try {
            $members = PersistenceContext::get_querier()->get_column_value(DB_TABLE_GROUP, 'members', 'WHERE id=:id', ['id' => NumberHelper::numeric($group['id'])]);
        } catch (RowNotFoundException $e) {}
