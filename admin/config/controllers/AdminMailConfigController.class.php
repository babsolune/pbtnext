...
class AdminMailConfigController extends DefaultAdminController
{
    ...
    protected function build_form()
    {
        ...
        $fieldset->add_field(new FormFieldMultiLineTextEditor('mail_signature', $this->lang['configuration.email.signature'], $this->config->get_mail_signature(),
            ['description' => $this->lang['configuration.email.signature.clue']]
        ));

        $smtp_enabled = $this->config->is_smtp_enabled();

        $fieldset = new FormFieldsetHTML('send_configuration', $this->lang['configuration.email.send.protocol'], ['description' => $this->lang['configuration.email.send.protocol.clue']]);
        $form->add_fieldset($fieldset);
        ...
        $fieldset = new FormFieldsetHTML('smtp_configuration', $this->lang['configuration.email.custom.smtp.configuration'],
        ...
        $fieldset->add_field(new FormFieldTextEditor('smtp_host', $this->lang['configuration.email.smtp.host'], $this->config->get_smtp_host(),
            ['required' => true, 'disabled' => !$smtp_enabled],
            [new FormFieldConstraintRegex('`^[a-z0-9-]+(?:\.[a-z0-9-]+)*$`iu')]
        ));

        $fieldset->add_field(new FormFieldNumberEditor('smtp_port', $this->lang['configuration.email.smtp.port'], $this->config->get_smtp_port(),
            ['min' => 1, 'max' => 65535, 'disabled' => !$smtp_enabled],
            [new FormFieldConstraintIntegerRange(0, 65535)]
        ));
        ...
        $fieldset->add_field(new FormFieldSimpleSelectChoice('smtp_protocol', $this->lang['configuration.email.smtp.secure.protocol'], $default_protocol_option,
            [$none_protocol_option, $tls_protocol_option, $ssl_protocol_option],
            ['disabled' => !$smtp_enabled]
        ));
    ...
    protected function save()
    {
        ...
        HooksService::execute_hook_action('edit_config', 'kernel', ['title' => $this->lang['configuration.email.sending'], 'url' => AdminConfigUrlBuilder::mail_config()->rel()]);
    }
}
?>
