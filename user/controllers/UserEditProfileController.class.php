...
class UserEditProfileController extends AbstractController
{
	...
	private function build_form()
	{
		...
		$fieldset->add_field($display_name = new FormFieldTextEditor('display_name', $this->lang['user.display.name'], $this->user->get_display_name(),
			...
			[new FormFieldConstraintLengthRange(3, 100), new FormFieldConstraintDisplayNameExists($this->user->get_id())]
		));
		...
		$fieldset->add_field($email = new FormFieldMailEditor('email', $this->lang['user.email'], $this->user->get_email(),
			...
			[new FormFieldConstraintMailExist($this->user->get_id())]
		));
		...
		$connect_fieldset->add_field($login = new FormFieldTextEditor('login', $this->lang['user.username'], ($has_custom_login ? $this->internal_auth_infos['login'] : preg_replace('/\s+/u', '', $this->user->get_display_name())),
			...
			[new FormFieldConstraintLengthRange(3, 25), new FormFieldConstraintPHPBoostAuthLoginExists($this->user->get_id())]
		));
		...
		$options_fieldset->add_field(new FormFieldTimezone('timezone', $this->lang['user.timezone.choice'],
			$this->user->get_timezone(), ['description' => $this->lang['user.timezone.choice.clue']]
		));
		...
		if (count(ThemesManager::get_activated_and_authorized_themes_map()) > 1)
		{
			$options_fieldset->add_field(new FormFieldThemesSelect('theme', $this->lang['user.theme'], $this->user->get_theme(),
				[
					...
					'events' => ['change' => $this->build_javascript_picture_themes()]
				]
			));
	...
	private function delete_account()
	{
		if ($this->user->get_level() != User::ADMINISTRATOR_LEVEL || ($this->user->get_level() == User::ADMINISTRATOR_LEVEL && $this->user->get_id() != AppContext::get_current_user()->get_id()) || ($this->user->get_level() == User::ADMINISTRATOR_LEVEL && $this->user->get_id() == AppContext::get_current_user()->get_id() && UserService::count_admin_members() > 1))
		{
			...
			HooksService::execute_hook_action('delete_user', 'user', array_merge($this->user->get_properties(), ['title' => $this->user->get_display_name()]));
		}

		if ($this->user->get_id() == AppContext::get_current_user()->get_id())
			AppContext::get_response()->redirect(Environment::get_home_page(), $this->lang['user.message.success.delete.member']);
		else
			AppContext::get_response()->redirect(UserUrlBuilder::home(), StringVars::replace_vars($this->lang['user.message.success.delete'], ['name' => $this->user->get_display_name()]));
	}
	...
	private function save(HTTPRequestCustom $request)
	{
		...
		$user_properties = array_merge($this->user->get_properties(), ['email' => $this->user->get_email()]);
		...
		if (AppContext::get_current_user()->is_admin())
		{
			...
			if ($old_level != $level)
				HooksService::execute_hook_action('user_change_level', 'user', array_merge($this->user->get_properties(), ['title' => $this->user->get_display_name(), 'url' => UserUrlBuilder::profile($user_id)->rel()]), UserService::get_level_lang($this->user->get_level()));
		}
		...
		if (AppContext::get_current_user()->is_admin())
		{
			if ($old_approbation != $approbation && $old_approbation == 0)
			{
				...
				if (count($matching_alerts) == 1)
				{
					...
					$subject = StringVars::replace_vars($this->lang['user.registration.email.subject'], ['site_name' => $site_name]);
					$content = StringVars::replace_vars($this->lang['user.registration.email.administrator.validation.content'], [
			...
			if (!empty($user_warning) && $user_warning != $this->user->get_warning_percentage())
			{
				...
				HooksService::execute_hook_action('user_warning', 'user', array_merge($this->user->get_properties(), ['title' => $this->user->get_display_name(), 'url' => UserUrlBuilder::profile($user_id)->rel(), 'warning_percentage' => $user_warning]), $user_warning . ' %');
			}
			...
			if (!empty($user_readonly) && $user_readonly != $this->user->get_delay_readonly())
			{
				...
				HooksService::execute_hook_action('user_punishment', 'user', array_merge($this->user->get_properties(), ['title' => $this->user->get_display_name(), 'url' => UserUrlBuilder::profile($user_id)->rel(), 'delay_readonly' => $user_readonly]), isset($sanctions_duration[$user_readonly]) ? $sanctions_duration[$user_readonly] : '');
			}
			...
			if (!empty($user_ban) && $user_ban != $this->user->get_delay_banned())
			{
				...
				HooksService::execute_hook_action('user_ban', 'user', array_merge($this->user->get_properties(), ['title' => $this->user->get_display_name(), 'url' => UserUrlBuilder::profile($user_id)->rel(), 'delay_banned' => $user_ban]), isset($sanctions_duration[$user_ban]) ? $sanctions_duration[$user_ban] : '');
			}
		...
		{
			$description = StringVars::replace_vars($this->lang['user.change.profile.field.description'], ['field' => $this->lang['user.display.name'], 'old_value' => $user_properties['display_name'], 'new_value' => $this->user->get_display_name()]);
			HooksService::execute_hook_action('user_change_display_name', 'user', array_merge($this->user->get_properties(), ['title' => $this->user->get_display_name(), 'url' => UserUrlBuilder::profile($user_id)->rel()]), $description);
		}

		if ($this->user->get_email() != $user_properties['email'])
		{
			$description = StringVars::replace_vars($this->lang['user.change.profile.field.description'], ['field' => $this->lang['user.email'], 'old_value' => $user_properties['email'], 'new_value' => $this->user->get_email()]);
			HooksService::execute_hook_action('user_change_email', 'user', array_merge($this->user->get_properties(), ['title' => $this->user->get_display_name(), 'url' => UserUrlBuilder::profile($user_id)->rel()]), $description);
		}
	...
	private function build_response()
	{
		...
		$breadcrumb->add(StringVars::replace_vars($this->lang['user.profile.of'], ['name' => $this->user->get_display_name()]), UserUrlBuilder::profile($this->user->get_id())->rel());
