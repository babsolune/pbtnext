...
//global PHP_MATH_PUBLISHER_FONT_DIR,PHP_MATH_PUBLISHER_CACHE_DIR,$symboles,$fontesmath;
...
function tableau_expression($expression)
{
    ...
    for ($i = $d; $i < count($t) - 1; $i++) {
        ...
        if ('<=' == $t[$i]) {
            ...
            if ('^' == $t[$i - 1] || '_' == $t[$i - 1] || '*' == $t[$i - 1] || '/' == $t[$i - 1] || '+' == $t[$i - 1] || '(' == $t[$i - 1]) {
                $t[$i] = '';
                if (is_array($t[$i + 1])) {
                    $t[$i + 1][1] = '-' . $t[$i + 1][1];
                } else {
    ...
    switch ($texte) {
        ...
        case '_tintegrale':
            if (isset($fontesmath[$texte])) {
                ...
                $font = PHP_MATH_PUBLISHER_FONT_DIR . "/cmmi10.ttf";
            }
            ...
            $tmp_blanc   = imagecolortransparent($tmp_img, $tmp_blanc);
            ImageFilledRectangle($tmp_img, 0, 0, $tmp_largeur, $tmp_hauteur, $tmp_blanc);
            ImageTTFText($tmp_img, $t, 0, 5, (int)($tmp_hauteur / 2), $tmp_noir, $font, $texte);
            $img       = $tmp_img;
            $toutblanc = true;
            $sx        = $sy        = $ex        = $ey        = -1;
            for ($y = 0; $y < $tmp_hauteur; $y++) {
...
class expression_math extends expression
{
            ...
            if ('(' == $this->noeuds[$i]->texte || '{' == $this->noeuds[$i]->texte) {
                ...
                if (count($parentheses) == 0) {
                    ...
                    if (')' == $this->noeuds[$i]->texte) {
                        $ret[] = new expression_math([new expression_texte("("), new expression_math($sub), new expression_texte(")")]);
                    } else {
     ...
     * @return mixed
     */
    public function traite_operation($noeuds, $operation)
    {
        do {
            $change = false;
            if (count($noeuds) <= 3) {
            ...
            for ($i = 0; $i < count($noeuds); $i++) {
                if (!$change && $i < count($noeuds) - 2 && $noeuds[$i + 1]->texte == $operation) {
                    $ret[] = new expression_math([$noeuds[$i], $noeuds[$i + 1], $noeuds[$i + 2]]);
                    $i += 2;
                    $change = true;
                } else {
                    $ret[] = $noeuds[$i];
     ...
     * @return mixed
     */
    public function traite_fonction($noeuds, $fonction, $nbarg)
    {
    ...
    public function dessine($taille)
    {
        switch (count($this->noeuds)) {
            ...
            case 2:
                ...
                break;
            case 3:
                if ("lim" == $this->noeuds[0]->texte) {
                    $this->dessine_limite($taille);
                } elseif ("root" == $this->noeuds[0]->texte) {
     ...
     * @param $taille
     */
    public function dessine_expression($taille)
    {
        $largeur = 1;
        $hauteur = 1;
        $dessus  = 1;
     ...
     * @param $taille
     */
    public function dessine_fraction($taille)
    {
        ...
        ImageCopy($result, $img1, (int) (($largeur - $largeur1) / 2), 0, 0, 0, $largeur1, $hauteur1);
        imageline($result, 0, $this->base_verticale, $largeur, $this->base_verticale, $noir);
        ImageCopy($result, $img2, (int) (($largeur - $largeur2) / 2), $hauteur1 + 4, 0, 0, $largeur2, $hauteur2);
        $this->image = $result;
    }
     ...
     * @param $taille
     */
    public function dessine_exposant($taille)
    {
        $this->noeuds[0]->dessine($taille);
        $img1  = $this->noeuds[0]->image;
        $base1 = $this->noeuds[0]->base_verticale;
        ...
        if ($hauteur1 >= $hauteur2) {
            ...
            $blanc                = imagecolortransparent($result, $blanc);
            ImageFilledRectangle($result, 0, 0, $largeur - 1, $hauteur - 1, $blanc);
            ImageCopy($result, $img1, 0, (int) ceil($hauteur2 / 2), 0, 0, $largeur1, $hauteur1);
            ...
            $blanc                = imagecolortransparent($result, $blanc);
            ImageFilledRectangle($result, 0, 0, $largeur - 1, $hauteur - 1, $blanc);
            ImageCopy($result, $img1, 0, (int) ceil($hauteur2 - $hauteur1 / 2), 0, 0, $largeur1, $hauteur1);
            ImageCopy($result, $img2, $largeur1, 0, 0, 0, $largeur2, $hauteur2);
        }
        $this->image = $result;
    }
     ...
     * @param $taille
     */
    public function dessine_indice($taille)
    {
        $this->noeuds[0]->dessine($taille);
        $img1  = $this->noeuds[0]->image;
        $base1 = $this->noeuds[0]->base_verticale;
        ...
        if ($hauteur1 >= $hauteur2) {
            ...
            ImageCopy($result, $img2, $largeur1, (int) ceil($hauteur1 - $hauteur2 / 2), 0, 0, $largeur2, $hauteur2);
        } else {
            $hauteur              = ceil($hauteur1 / 2 + $hauteur2);
            $this->base_verticale = $base1;
            $result               = ImageCreate(max($largeur, 1), max($hauteur, 1));
            ...
            ImageFilledRectangle($result, 0, 0, $largeur - 1, $hauteur - 1, $blanc);
            ImageCopy($result, $img1, 0, 0, 0, 0, $largeur1, $hauteur1);
            ImageCopy($result, $img2, $largeur1, (int) ceil($hauteur1 / 2), 0, 0, $largeur2, $hauteur2);
        }
        $this->image = $result;
    }
     ...
     * @param $taille
     */
    public function dessine_racine($taille)
    {
     ...
     * @param $taille
     */
    public function dessine_root($taille)
    {
     ...
     * @param $caractere
     */
    public function dessine_grandoperateur($taille, $caractere)
    {
        ...
        ImageCopy($imggauche, $imgsymbole, (int)(($largeurgauche - $largeursymbole) / 2), $hauteur2, 0, 0, $largeursymbole, $hauteursymbole);
        ImageCopy($imggauche, $img2, (int)(($largeurgauche - $largeur2) / 2), 0, 0, 0, $largeur2, $hauteur2);
        ImageCopy($imggauche, $img1, (int)(($largeurgauche - $largeur1) / 2), $hauteur2 + $hauteursymbole, 0, 0, $largeur1, $hauteur1);
     ...
     * @param $taille
     */
    public function dessine_dessus($taille)
    {
        $this->noeuds[2]->dessine($taille * 0.8);
        ...
        ImageCopy($imgfin, $imgsup, (int)(($largeur - $largeursup) / 2), 0, 0, 0, $largeursup, $hauteursup);
        ImageCopy($imgfin, $imgexp, (int)(($largeur - $largeurexp) / 2), $hauteursup, 0, 0, $largeurexp, $hauteurexp);
        $this->image          = $imgfin;
     ...
     * @param $taille
     */
    public function dessine_dessous($taille)
    {
        $this->noeuds[2]->dessine($taille * 0.8);
        ...
        $blanc   = imagecolortransparent($imgfin, $blanc);
        ImageFilledRectangle($imgfin, 0, 0, $largeur - 1, $hauteur - 1, $blanc);
        ImageCopy($imgfin, $imgexp, (int)(($largeur - $largeurexp) / 2), 0, 0, 0, $largeurexp, $hauteurexp);
        ImageCopy($imgfin, $imginf, (int)(($largeur - $largeurinf) / 2), $hauteurexp, 0, 0, $largeurinf, $hauteurinf);
        $this->image          = $imgfin;
     ...
     * @param $taille
     */
    public function dessine_matrice($taille)
    {
        $padding      = 8;
        ...
        for ($ligne = 0; $ligne < $nbligne; $ligne++) {
            ...
            for ($col = 0; $col < $nbcolonne; $col++) {
                if ($i < count($this->noeuds[3]->noeuds)) {
                    ImageCopy($imgfin, $img[$i], $l + (int)(ceil($largeur_colonne[$col] - $largeur[$i]) / 2), $h + $dessus_ligne[$ligne] - $base[$i], 0, 0, $largeur[$i], $hauteur[$i]);
     ...
     * @param $taille
     */
    public function dessine_tableau($taille)
    {
        $padding      = 8;
        ...
        for ($ligne = 0; $ligne < $nbligne; $ligne++) {
            ...
            if (substr($typecolonne, 0, 1) == "1") {
                ImageLine($imgfin, 0, (int)($h - $padding / 2), 0, (int)($h + $hauteur_ligne[$ligne] + $padding / 2), $noir);
            }
            ...
            for ($col = 0; $col < $nbcolonne; $col++) {
                if ($i < count($this->noeuds[3]->noeuds)) {
                    ...
                    if (substr($typecolonne, $col + 1, 1) == "1") {
                        ImageLine($imgfin, (int)($l + $largeur_colonne[$col] + $padding / 2), (int)($h - $padding / 2), (int)($l + $largeur_colonne[$col] + $padding / 2), (int)($h + $hauteur_ligne[$ligne] + $padding / 2), $noir);
                    }
            ...
            if (substr($typeligne, $ligne + 1, 1) == "1") {
                ImageLine($imgfin, 0, (int)($h + $hauteur_ligne[$ligne] + $padding / 2), $largeurfin - 1, (int)($h + $hauteur_ligne[$ligne] + $padding / 2), $noir);
            }
     ...
     * @param $taille
     */
    public function dessine_vecteur($taille)
    {
        // expression
        $this->noeuds[1]->dessine($taille);
        $imgexp     = $this->noeuds[1]->image;
     ...
     * @param $taille
     */
    public function dessine_overline($taille)
    {
        // expression
        $this->noeuds[1]->dessine($taille);
        $imgexp     = $this->noeuds[1]->image;
     ...
     * @param $taille
     */
    public function dessine_underline($taille)
    {
        // expression
        $this->noeuds[1]->dessine($taille);
        $imgexp     = $this->noeuds[1]->image;
     ...
     * @param $taille
     */
    public function dessine_chapeau($taille)
    {
     ...
     * @param $taille
     */
    public function dessine_limite($taille)
    {
        ...
        $blanc   = imagecolortransparent($imgfin, $blanc);
        ImageFilledRectangle($imgfin, 0, 0, $largeur - 1, $hauteur - 1, $blanc);
        ImageCopy($imgfin, $imglim, (int)(($largeur - $largeurlim) / 2), 0, 0, 0, $largeurlim, $hauteurlim);
        ImageCopy($imgfin, $imginf, (int)(($largeur - $largeurinf) / 2), $hauteurlim, 0, 0, $largeurinf, $hauteurinf);
     ...
     * @param $taille
     */
    public function dessine_delimiteur($taille)
    {
        $this->noeuds[2]->dessine($taille);
        $imgexp     = $this->noeuds[2]->image;
        $baseexp    = $this->noeuds[2]->base_verticale;
        $hauteurexp = imagesy($imgexp);
        if ("&$" == $this->noeuds[1]->texte && isset($this->noeuds[1]->noeuds[0])) {
...
function detectimg($n)
{
     ...
     */
    $ret    = 0;
    $handle = opendir(PHP_MATH_PUBLISHER_CACHE_DIR);
    while ($fi = readdir($handle)) {
        $info = pathinfo($fi);
        if ("." != $fi && ".." != $fi && "png" == $info["extension"] && @preg_match('/^math/', $fi)) {
...
function mathfilter($text, $size, $pathtoimg)
{
    ...
    - if the image corresponding to the formula doesn't exist in the PHP_MATH_PUBLISHER_CACHE_DIR cache directory (detectimg($nameimg)=0), the script creates the image and returns the "<img src=...></img>" code.
