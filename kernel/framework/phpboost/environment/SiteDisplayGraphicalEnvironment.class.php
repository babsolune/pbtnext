...
class SiteDisplayGraphicalEnvironment extends AbstractDisplayGraphicalEnvironment
{
	...
	function display_page(View $body_template)
	{
		...
		foreach ($seo_meta_data->get_additionnal_properties() as $og_id => $og_value)
		{
			if (is_array($og_value))
			{
				...
				{
					$view->assign_block_vars('og_additionnal_properties', [
						'ID' 	=> $og_id,
						'VALUE' => $og_sub_value
					]);
				}
	...
	protected function display_counter(Template $view)
	{
		...
		{
			try {
				$visit_counter = PersistenceContext::get_querier()->select_single_row(DB_TABLE_VISIT_COUNTER, ['ip AS nbr_ip', 'total'], 'WHERE id = 1');
			} catch (RowNotFoundException $e) {
				$visit_counter = ['nbr_ip' => 1, 'total' => 1];
			}
	...
	protected function display_site_maintenance()
	{
		...
		if ($maintenance_config->is_under_maintenance() && $maintenance_config->get_display_duration() && (!AppContext::get_current_user()->is_admin() || (AppContext::get_current_user()->is_admin() && $maintenance_config->get_display_duration_for_admin())))
		{
			...
			$array_time = [-1, 60, 300, 600, 900, 1800, 3600, 7200, 10800, 14400, 18000, 21600, 25200, 28800, 57600, 86400, 172800, 604800];
			$array_delay = [
				self::$lang['common.unspecified'],
				'1 ' . self::$lang['date.minute'],
				'5 ' . self::$lang['date.minutes'],
				'10 ' . self::$lang['date.minutes'],
				'15 ' . self::$lang['date.minutes'],
				'30 ' . self::$lang['date.minutes'],
				'1 ' . self::$lang['date.hour'],
				'2 ' . self::$lang['date.hours'],
				'3 ' . self::$lang['date.hours'],
				'4 ' . self::$lang['date.hours'],
				'5 ' . self::$lang['date.hours'],
				'6 ' . self::$lang['date.hours'],
				'7 ' . self::$lang['date.hours'],
				'8 ' . self::$lang['date.hours'],
				'16 ' . self::$lang['date.hours'],
				'1 ' . self::$lang['date.day'],
				'2 ' . self::$lang['date.day'],
